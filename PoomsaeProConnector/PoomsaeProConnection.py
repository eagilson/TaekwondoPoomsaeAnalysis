...
def formatPP_ScoresV2c(data, DatabaseID, tablename, event, start_date, databasename):
    ...
    if databasename == 'PP_ScoresV2c.accdb' and tablename == 'DivisionNames' and event != '2025 USATKD Team Trials':
        df = pd.DataFrame(datawid)
    ...
    #V1c database DivisionName updates
    if databasename == 'PP_ScoresV1c.accdb' and tablename == 'DivisionNames':
        df = pd.DataFrame(datawid)
        df['CompMth_Black'] = df.iloc[:,6]
        df['CompOrder'] = df.iloc[:,4]
        newdf = df.iloc[:,[0,1,2,3,9,8,6,4,5]]
        #df[[0L,1L,2L,3L,'CompOrder','CompMeth_Black',6L,4L,5L,7L]]
        datawid = [tuple(row) for row in newdf.to_numpy()]

    #code for V3c update on 2025-07
    if start_date < '2025-07-01' and databasename == 'PP_ScoresV3c.accdb' and tablename == 'SEMatchList':
...
def extractPP_ScoresV2c(DatabaseID, database):
    try:
        ...
        for i in curevent.tables():
            ...
            #Normalize SQL Table names
            tablename = i.table_name.replace(" ", "")

            #Division Names & Round to TT versions from TT database
            if tablename in ('DivisionNames', 'Round') and database['databasename'] == 'PP_ScoresV4t.accdb':
                tablename = 'TT' + tablename
            
            #Add Tbl to Round, Category, Gender to prevent overlap with fieldnames
